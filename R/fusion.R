...
tidy_fusion <- function(tsv_file) {
  data <- read_delim(x, delim = "\t", comment = "#")
  data <- data %>%
    filter(rowtype %in% c("Fusion", "RNAExomVariant")) %>%
    filter(INFO.1.GENE_NAME %in% fusion_df$Gene) %>%
    select(rowtype, INFO.1.GENE_NAME, ID)
  data
}
...
make.fusion.df <- function(list_tsv_file){
  ...
  for (i in list_tsv_file){
  data <- tidy_fusion(i)
  df_temp <- tibble(Hugo_Symbol = data$INFO.1.GENE_NAME,
                      Entrez_Gene_Id= purrr::map_dbl(data$INFO.1.GENE_NAME, find.Entres_Gene_Id),
                      Center = rep("cmc", dim(data)[1]),
                      Tumor_Sample_Barcode = sample_ID,
                      Fusion = data$ID,
                      DNA_support = rep("no", dim(data)[1]),
                      RNA_support = rep("yes", dim(data)[1]),
                      Method = rep("OCAv3", dim(data)[1]),
