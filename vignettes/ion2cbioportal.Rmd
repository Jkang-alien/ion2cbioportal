...
Because the cbioportal maf file should includes all data in one maf file, maf files of multiple samples must be merged. The `all_maf(/path/to/maf/need/to/merge)` generates merged maf file (`output/directory/all_maftools.maf`). This is the final cbioportal maf data file.  
...
##### 6. Import cbioportal data  

If cbioportal is deploied with thehyve docker image, follow the instructions. https://github.com/thehyve/cbioportal-docker/blob/master/docs/example_commands.md `-o` option means overriding warnings.  


```{}
cd ./cbioportal_data/study-dir/

docker run -it --rm --net cbio-net \
    -v /home/molpath/cbioportal-docker/portal.properties:/cbioportal/portal.properties:ro \
    -v "$PWD/:/study:ro" \
    -v "$HOME/Desktop:/outdir" \
    cbioportal-image \
    metaImport.py -o -u http://cbioportal-container:8080/cbioportal -s /study --html=/outdir/report.html
...
1. The thehyve is better than the cbioportal documentation for deploing cbioportal using docker. The cbioportal documentation makes problem in migration of database to latest version.  

2. If migration of database to latest version fails, the undefined cancer type error will follow.  
